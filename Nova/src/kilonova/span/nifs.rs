...
impl<E: Engine> NIFS<E> {
  ...
  pub fn dummy(
    ...
    Self::absorb_inputs(&mut ro, pp_digest, predicate_id, U2);
  ...
  pub fn prove(
    ...
    let L0 = Self::row_table_eval_folded(structure, &U1.lambda, 0, &U1.r_y);
    let L1 = Self::row_table_eval_folded(structure, &U1.lambda, 1, &U1.r_y);
    let L2 = Self::row_table_eval_folded(structure, &U1.lambda, 2, &U1.r_y);
    ...
    let sigmas = structure.compute_sigmas(U1, &r_x_prime);
    ...
    let R0 = Self::col_table_eval_folded(structure, &U1.lambda, 0, &r_x_prime);
    let R1 = Self::col_table_eval_folded(structure, &U1.lambda, 1, &r_x_prime);
    let R2 = Self::col_table_eval_folded(structure, &U1.lambda, 2, &r_x_prime);
    ...
    let epsilons = structure.compute_epsilons(U1, &z_running, &r_x_prime, &r_y_prime);
    ...
    let W = W1.fold(structure, W2, &rho_base, &rho_scalar)?;
...
mod tests {
  ...
  fn test_nifs_roundtrip() {
    ...
    let W1 = super::super::relation::FoldedWitness::from_r1cs_witness(&S, &W0);
