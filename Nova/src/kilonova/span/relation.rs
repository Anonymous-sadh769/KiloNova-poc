...
impl<E: Engine> GlobalStructure<E> {
  ...
  fn build(shapes: &[R1CSShape<E>], with_runtime_tables: bool) -> Self {
    ...
    let ck_index: [CommitmentKey<E>; 3] =
      core::array::from_fn(|j| E::CE::setup(INDEX_LABELS[j], shapes.len()));
  ...
  pub(crate) fn compute_sigmas(
    ...
    (0..3)
      .map(|j| self.eval_matrix_folded(&U.lambda, j, r_x_prime, &U.r_y))
      .collect()
  }
  ...
  pub(crate) fn compute_epsilons(
    ...
    vec![
      self.eval_matrix_folded(&U.lambda, 0, r_x_prime, r_y_prime),
      self.eval_matrix_folded(&U.lambda, 1, r_x_prime, r_y_prime),
      self.eval_matrix_folded(&U.lambda, 2, r_x_prime, r_y_prime),
  ...
  pub(crate) fn reconstruct_folded_index_commitments(
    &self,
    lambda: &[E::Base],
  ) -> [Commitment<E>; 3] {
    debug_assert_eq!(lambda.len(), self.predicates.len());
    let coeffs = lambda
      .iter()
      .copied()
      .map(base_as_scalar::<E>)
      .collect::<Vec<_>>();

    core::array::from_fn(|which| {
      coeffs
        .iter()
        .zip(self.predicates.iter())
        .fold(Commitment::<E>::default(), |acc, (coeff, predicate)| {
          acc + predicate.comm_M[which] * *coeff
        })
    })
  }
  ...
  pub fn is_sat(
    ...
    if W.W.len() != self.num_vars_max
      ...
      || U.lambda.len() != self.predicates.len()
      || U.X.len() != self.num_io
    ...
    let expected_v = self.compute_v_from_coeffs(&U.lambda, U.u, &U.r_x, &U.r_y, &z);
    if expected_v != U.v {
      return Err(NovaError::UnSat {
...
impl<E: Engine> FoldedWitness<E> {
  ...
  pub fn from_r1cs_witness(S: &GlobalStructure<E>, witness: &R1CSWitness<E>) -> Self {
...
impl<E: Engine> FoldedInstance<E> {
  pub fn default(S: &GlobalStructure<E>) -> Self {
    Self {
      ...
      lambda: vec![E::Base::ZERO; S.predicates.len()],
      u: E::Base::ZERO,
...
impl<E: Engine> TranscriptReprTrait<E::GE> for FoldedInstance<E> {
  fn to_transcript_bytes(&self) -> Vec<u8> {
    [
      ...
      self.lambda.as_slice().to_transcript_bytes(),
...
mod tests {
  ...
  fn test_folded_relation_sat() {
    ...
    let folded_w = FoldedWitness::from_r1cs_witness(&S, &W);
    let folded_u = FoldedInstance::from_r1cs_instance_witness(&S, 0, &U, &W);
    assert!(S.is_sat(&ck0, &folded_u, &folded_w).is_ok());
    assert_eq!(folded_u.lambda, S.basis_coeffs_base(0));
  }

  #[test]
  fn test_folded_lambda_update() {
    let (shape0, ck0) = tiny_r1cs::<PallasEngine>(<PallasEngine as Engine>::Scalar::ONE);
    let (shape1, _) = tiny_r1cs::<PallasEngine>(<PallasEngine as Engine>::Scalar::from(2u64));
    ...
    let running_w = FoldedWitness::from_r1cs_witness(&S, &W0);
    ...
    let folded_w = running_w.fold(&S, &W1, &rho_base, &rho_scalar).unwrap();
    let z = S.z_from_folded(&running_u, &folded_w);
    let folded_u = running_u.fold(
      ...
      S.compute_v_from_coeffs(
        &[<PallasEngine as Engine>::Base::ONE, rho_base],
    ...
    assert_eq!(folded_u.lambda[0], <PallasEngine as Engine>::Base::ONE);
    assert_eq!(folded_u.lambda[1], rho_base);
    assert_eq!(folded_w.W_proj.len(), S.num_vars_max);
  }
}
