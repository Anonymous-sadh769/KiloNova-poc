...
pub mod r1cs;
use r1cs::{
  AllocatedFoldedInstance, AllocatedNIFS, AllocatedProjectedWitness, AllocatedR1CSInstance,
};
...
mod tests {
  use crate::{
    ...
    kilonova::test_utils::{
      build_direct_secondary_case, build_span_secondary_case,
      build_span_secondary_single_step_snark, synthesize_secondary_direct,
      synthesize_secondary_span, PrimaryBase, SecondaryEngine,
    },
  };
  use ff::Field;
  ...
  fn test_secondary_augmented_non_base_case_satisfiable() {
    let case = build_span_secondary_case();
    ...
    synthesize_secondary_span(&case, &mut test_cs).unwrap();
    assert!(
      test_cs.is_satisfied(),
      "span circuit unsatisfied: {:?}",
      test_cs.which_is_unsatisfied()
    );
  }

  #[test]
  fn test_secondary_augmented_rejects_bad_predicate_id() {
    let mut case = build_span_secondary_case();
    case.incoming_predicate_id_runtime = PrimaryBase::ZERO;
  ...
  #[test]
  fn test_secondary_augmented_direct_vs_span_constraint_counts() {
    let direct_case = build_direct_secondary_case();
    let span_case = build_span_secondary_case();

    let mut direct_cs = ShapeCS::<SecondaryEngine>::new();
    synthesize_secondary_direct(&direct_case, &mut direct_cs).unwrap();
    let mut span_cs = ShapeCS::<SecondaryEngine>::new();
    synthesize_secondary_span(&span_case, &mut span_cs).unwrap();

    eprintln!(
      "direct constraints={} aux={} inputs={}",
      direct_cs.num_constraints(),
      direct_cs.num_aux(),
      direct_cs.num_inputs()
    );
    eprintln!(
      "span constraints={} aux={} inputs={}",
      span_cs.num_constraints(),
      span_cs.num_aux(),
      span_cs.num_inputs()
    );

    assert!(span_cs.num_constraints() < direct_cs.num_constraints());
  }

  #[test]
  fn test_reconstruct_folded_index_commitments_matches_direct() {
