...
impl<E: Engine> AllocatedFoldedInstance<E> {
  pub fn alloc<CS: ConstraintSystem<E::Base>>(
    ...
    let lambda = alloc_num_vec::<E, _>(
      cs.namespace(|| "allocate lambda"),
      inst.map(|inst| &inst.lambda),
  ...
  pub fn from_r1cs_instance_witness<CS: ConstraintSystem<E::Base>>(
    ...
    let lambda = alloc_num_vec::<E, _>(
      cs.namespace(|| "base_lambda"),
      Some(&structure.basis_coeffs_base(predicate_id)),
      structure.predicates.len(),
      "base_lambda",
    )?;
  ...
  pub fn absorb_in_ro<CS: ConstraintSystem<E::Base>>(
    ...
    for (j, coeff) in self.lambda.iter().enumerate() {
      let _ = j;
      ro.absorb(coeff);
    }
    ro.absorb(&self.u);
  ...
  pub fn fold_with_r1cs_holographic<CS: ConstraintSystem<E::Base>>(
    ...
    let lambda = self
      ...
      .map(|(i, coeff)| {
        if i == predicate_id {
          coeff.add(cs.namespace(|| format!("lambda_fold_{i}")), &rho)
        } else {
          Ok(coeff.clone())
        }
      })
  ...
  pub fn conditionally_select<CS: ConstraintSystem<E::Base>>(
    ...
    Ok(Self {
      ...
      lambda: self
        .lambda
        .iter()
        .zip(other.lambda.iter())
        .enumerate()
        .map(|(i, (a, b))| {
          conditionally_select(
            cs.namespace(|| format!("lambda select {i}")),
            a,
            b,
            condition,
          )
