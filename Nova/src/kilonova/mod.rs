...
use circuit::{KiloNovaAugmentedCircuit, KiloNovaAugmentedCircuitInputs};
use nifs::NIFS;
  ...
  PS: PredicateSet<E1::Scalar>,
{
  pub fn new(
    ...
    let inputs_primary = KiloNovaAugmentedCircuitInputs::new(
      ...
      Some(vec![
        E1::Scalar::ZERO;
        pp.structure_secondary.predicates.len()
      ]),
      None,
      ri_primary,
    ...
    let r_W_primary =
      FoldedWitness::from_r1cs_witness(&pp.structure_primary, first_predicate_id, &w_primary);
    ...
    let inputs_secondary = KiloNovaAugmentedCircuitInputs::new(
      ...
      Some(bootstrap_running_W_primary.lambda_proj.clone()),
      None,
      ri_secondary,
  ...
  pub fn prove_step(
    ...
    let inputs_primary = KiloNovaAugmentedCircuitInputs::new(
      ...
      Some(self.r_W_secondary.lambda_proj.clone()),
    ...
    let inputs_secondary = KiloNovaAugmentedCircuitInputs::new(
      ...
      Some(self.r_W_primary.lambda_proj.clone()),
...
mod tests {
  ...
  impl<F: PrimeField> ExamplePredicates<F> {
    fn output(&self, predicate_id: usize, z: &[F]) -> Vec<F> {
      match predicate_id {
        0 => vec![z[0] * z[0] * z[0] + z[0] + F::from(5u64)],
        1 => vec![z[0] * z[0] + z[0] + F::from(7u64)],
        _ => vec![z[0] * z[0] * z[0] * z[0] + F::from(9u64)],
      }
    }
  }
  ...
  impl<F: PrimeField> PredicateSet<F> for ExamplePredicates<F> {
    ...
    fn synthesize<CS: ConstraintSystem<F>>(
      ...
      match predicate_id {
        ...
        _ => {
          let x_sq = x.square(cs.namespace(|| "x_sq"))?;
          let x_4 = x_sq.square(cs.namespace(|| "x_4"))?;
          let y = AllocatedNum::alloc(cs.namespace(|| "y"), || {
            Ok(x_4.get_value().unwrap() + F::from(9u64))
          })?;
          cs.enforce(
            || "y = x^4 + 9",
            |lc| lc + x_4.get_variable() + (F::from(9u64), CS::one()),
            |lc| lc + CS::one(),
            |lc| lc + y.get_variable(),
          );
          Ok(vec![y])
  ...
  fn test_ivc_nontrivial_nonuniform_smoke() {
    run_nonuniform_trace_test(&[0usize, 2usize]);
  }

  #[test]
  fn test_ivc_nontrivial_nonuniform() {
    run_nonuniform_trace_test(&[0usize, 1usize, 2usize]);
  }
}
